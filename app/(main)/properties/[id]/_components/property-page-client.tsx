...
export function PropertyPageInner({
  id,
  property,
  initialImages,
  session,
  ...
  // Subscription data is user-specific — must be client-side
  ...
  const rawImages =
    initialImages?.length > 0
      ? initialImages.map((img: any) => img.url).filter(Boolean)
      : FALLBACK_IMAGES;
    ...
    <div className="min-h-screen bg-background text-foreground">
        ...
        <div className="max-w-screen-xl mx-auto px-4 md:px-8 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Button
              ...
              // class/Name="text-sm font-medium text-muted-foreground hover:text-foreground"
            >
              <ArrowLeft size={16} />
              Go Back
            </Button>
      ...
      <div className="max-w-screen-xl mx-auto px-4 md:px-8 py-6 md:py-10">
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_380px] gap-6 lg:gap-8 items-start">
          {/* ── Gallery — order 2 on mobile, 1 on desktop ── */}
                  ...
                  <div className="bg-card/95 backdrop-blur-xl p-8 rounded-2xl shadow-2xl flex flex-col items-center text-center border border-border max-w-[300px]">
                    ...
                    {totalCredits > 0 ? (
                      <Button
                        size="sm"
                        disabled={
                          subscriptionLoading || consumeContactCredit.isPending
                        }
          ...
          {/* ── Info Panel — order 1 on mobile, 2 on desktop ── */}
          <div className="flex flex-col gap-5 order-1 lg:order-2 lg:sticky lg:top-[88px] lg:self-start lg:max-h-[calc(100vh-100px)] lg:overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <div>
            ...
            {!isAnonymous && (
              <div className="space-y-2">
                {premiumFeatures.map(({ icon: Icon, label, desc, href }) => {
                  const canAccess = isAdmin || alreadyUnlocked;
                  return (
                    <Link
                      key={label}
                      ...
                      onClick={(e) => {
                        if (!canAccess) {
                          ...
                          if (isGuest)
                            router.push(`/login?redirectTo=/properties/${id}`);
                          else if (totalCredits > 0) handleContactClick();
                          else router.push(`/properties/${id}/contact`);
                        }
                      }}
                    >
                      <div
                ...
                <div className="rounded-2xl border border-border/60 bg-muted/20 p-5 flex flex-col items-center text-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-muted-foreground leading-relaxed">
                      Log in to book a visit, contact the seller, and save this
                      property.
                    </p>
                  </div>
                  <div className="flex flex-col gap-2 w-full">
                    ...
                    <Link
                      href={`/register?redirectTo=/properties/${id}`}
                      className="w-full"
                    >
                      <Button
                        variant="outline"
...
export default function PropertyPageClient(props: Props) {
  return (
    <Suspense fallback={<PropertyPageSkeleton />}>
      <PropertyPageInner {...props} />
    </Suspense>
  );
}
