  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 space-y-5">
                ...
                <table className="w-full text-sm min-w-[640px]">
                  <thead className="sticky top-0 z-10 bg-muted/60 backdrop-blur-sm">
                    <tr className="border-b border-border/60">
                      ...
                      <th className="text-left px-4 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider w-[12%]">
                        Role
                      </th>
                      <th className="text-left px-4 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider w-[14%]">
                        Status
                      </th>
                      <th className="text-left px-4 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider w-[22%]">
                        Joined
                      </th>
                  ...
                  <tbody className="divide-y divide-border/40">
                    {filtered.map((user) => (
                      <UserRow
                        ...
                        user={user}
                        onAction={(type) =>
                          setDialog({ type, user } as ActiveDialog)}
                      />
                    ))}
                  </tbody>
                </table>
      ...
      <ConfirmDialog
        ...
        description={`${
          dialog?.type === "unban" ? dialog.user.name : ""
      ...
      <ConfirmDialog
        ...
        description={`A reset link will be sent to ${
          dialog?.type === "reset" ? dialog.user.email : ""
        }.`}
      ...
      <ConfirmDialog
        ...
        description={`This will permanently delete ${
          dialog?.type === "delete" ? dialog.user.name : ""
...
function UserRow(
  { user, onAction }: {
    user: AdminUser;
    onAction: (
      type: "role" | "ban" | "unban" | "reset" | "impersonate" | "delete",
    ) => void;
  },
) {
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-48">
            ...
            {user.banned
              ? (
                <DropdownMenuItem
                  className="gap-2 cursor-pointer text-emerald-600 focus:text-emerald-600 dark:text-emerald-400"
...
function ConfirmDialog(
  {
    ...
    userId: string | null;
    onClose: () => void;
    action: "unban" | "reset" | "delete";
  },
) {
  const unban = useUnbanUser();
  const reset = useResetPassword();
  const deleteUser = useDeleteUser();
  const isPending = action === "unban"
    ? unban.isPending
    : action === "reset"
    ? reset.isPending
      ...
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            <span className="font-semibold">{user?.name}</span>{" "}
            ({user?.email}). You can stop at any time.
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>Cancel</Button>
          ...
          >
            {loading
              ? <Loader2 className="h-4 w-4 animate-spin mr-1.5" />
              : <LogIn className="h-4 w-4 mr-1.5" />}Start Impersonating
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
