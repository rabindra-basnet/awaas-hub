      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <CardHeader className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              {charts.appointmentsByStatus.map((_entry, idx) => (
                <input key={idx} type="color" value={appointmentColors[idx] || "#4CAF50"}
                  onChange={(e) => setAppointmentColors((prev) => { const c = [...prev]; c[idx] = e.target.value; return c; })}
                  title={_entry._id} className="w-6 h-6 rounded border" />
              ))}
            </div>
          </CardHeader>
          <CardContent>
              ...
              <BarChart data={charts.appointmentsByStatus}>
                ...
                <Bar dataKey="count">
                  {charts.appointmentsByStatus.map((_entry, index) => (
                    <Cell key={index} fill={appointmentColors[index] || "#4CAF50"} className="transition-all" />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <Card className="shadow">
          <CardHeader className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              {charts.propertiesByStatus.map((_entry, idx) => (
                <input key={idx} type="color" value={propertyColors[idx] || "#2196F3"}
                  onChange={(e) => setPropertyColors((prev) => { const c = [...prev]; c[idx] = e.target.value; return c; })}
                  title={_entry._id} className="w-6 h-6 rounded border" />
              ))}
            </div>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie data={charts.propertiesByStatus} dataKey="count" nameKey="_id" cx="50%" cy="50%" outerRadius={100} label>
                  {charts.propertiesByStatus.map((_entry, index) => (
                    <Cell key={index} fill={propertyColors[index] || "#2196F3"} className="transition-all" />
                  ))}
                </Pie>
                <Tooltip />
                <Legend />
