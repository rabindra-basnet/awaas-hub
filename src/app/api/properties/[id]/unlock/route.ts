...
/** Check if user has access and how many credits remain */
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  if (isAdmin || isOwner) {
    return NextResponse.json({ unlocked: true, remainingCredits: null, isOwner });
  }

  const [access, credAgg] = await Promise.all([
    PropertyContactAccess.findOne({ userId: session.user.id, propertyId: id }).lean(),
  ...
  return NextResponse.json({
    unlocked: !!access,
    remainingCredits: credAgg[0]?.remaining ?? 0,
    isOwner: false,
  });
}

/** Spend 1 credit to unlock this property */
export async function POST(_req: NextRequest, { params }: Params) {
  const { id } = await params;
  const session = await getServerSession();
  if (!session) return unauthorized();
  if ((session.user as any).isAnonymous) return forbidden("Login required");
  ...
  const isAdmin = session.user.role === Role.ADMIN;
  const isOwner = property.sellerId.toString() === session.user.id;
  if (isAdmin || isOwner) return NextResponse.json({ unlocked: true, remainingCredits: null });
  ...
  if (existing) {
    ...
    return NextResponse.json({ unlocked: true, remainingCredits: agg[0]?.remaining ?? 0 });
  }
  ...
  try {
    ...
    return NextResponse.json({ unlocked: true, remainingCredits });
  } catch (err: any) {
    if (err.message === "no_credits") return forbidden("No credits remaining. Purchase credits to unlock property details.");
    throw err;
  } finally {
